...
UNHELPFUL_LOCATIONS = (
    ...
    f"/inspect.py",
    f"/re.py",
    f"/re/__init__.py",  # refactored in Python 3.11
    f"/warnings.py",
    # Quite rarely, the first AFNP line is in Pytest's internals.
    f"/_pytest/_io/saferepr.py",
    f"/_pytest/assertion/*.py",
    f"/_pytest/config/__init__.py",
    f"/_pytest/pytester.py",
    f"/pluggy/_*.py",
    f"/reprlib.py",
    f"/typing.py",
)
...
def _glob_to_re(locs):
    ...
    # like our "**"
    return "|".join(
        loc.replace("**", "\0")  # handle ** below, after single
        .replace("*", r"[^/]+")
        .replace(".", re.escape("."))
        .replace("\0", ".*")
